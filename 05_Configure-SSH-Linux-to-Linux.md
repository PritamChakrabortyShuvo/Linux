...
## What is SSH?
**SSH** or **Secure Shell** is a network protocol that allows secure communication between two computers. It is commonly used for **accessing** & **managing** servers remotely.
## SSH Architecture
...
Here is the detailed steps :
  ...
  - The **client generates a random session key** & **encrypt it with the server’s public key**.
  - The **server decrypts the session key** using its **private key**.
  - The connection between the **client and the server** is now secured by encrypting each data using the session key.
  
In short, the client & server exchange keys and secure the connection for safe communication.
...
Here is the detailed steps :
  ...
  - If the client’s public key is not authorized the server rejects it & notify the client.
  - The client tries to **authenticate using the username** & **password**.
  - The server validates the provided credentials.
  - The client and servers can now exchange data & commands in a secured encrypted way.
...
We can restrict SSH access to specific users by using the **`AllowUsers directive`**. Replace **User_name01** & **User_name02** with the actual usernames we want to allow SSH access.
```bash
...
```bash
    sudo ufw allow from any to any port 1646 proto tcp
```
